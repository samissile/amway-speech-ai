...
# Create startup script that handles VPN connection
...
cat > /etc/openvpn/config/client.conf << OVPN
client
dev tun
proto tcp
remote sg-sng.prod.surfshark.com 1443
remote-random
nobind
tun-mtu 1500
mssfix 1450
...
openvpn --config /etc/openvpn/config/client.conf --daemon --log /var/log/openvpn.log
