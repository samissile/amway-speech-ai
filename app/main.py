...
# ✅ FIXED: Cross-platform directories (Windows/Linux compatible)
...
async def startup_event():
    await init_db()
    await cleanup_old_tasks(days_old=7)  # Cleanup old tasks
    asyncio.create_task(periodic_cleanup())  # Start background cleanup
    logger.info("✅ App started with memory optimizations")
...
async def transcribe(
    ...
    # ✅ FIXED: Use os.path.join for cross-platform paths
    temp_filename = f"upload_{secrets.token_hex(8)}_{file.filename}"
    temp_path = os.path.join(UPLOAD_DIR, temp_filename)
    file_size = 0
    
    try:
        # ✅ STREAM to disk (8KB chunks at a time)
